...
public class ConceptGraphDirectorClientTest {
	...
	public void testConceptGraphOfKResourceAssessment() {
		...
		ConceptGraphBuilder cgBuilder = new ConceptGraphBuilder(controller);
		cgBuilder.buildGraph(r);
		cgBuilder.buildEdges(r);
		Graph cg = cgBuilder.getGraph();
		
		Assert.assertEquals(expectedNumOfEdges, cg.getEdges().size());
		
		cgBuilder.buildNodes(r);
